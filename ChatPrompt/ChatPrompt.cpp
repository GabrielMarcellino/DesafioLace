...
void main()
{
    ...
    // Get the port that will be read
    ...
    //Specify ip and port to write to
    sockaddr_in server;
    ...
    while (1) {
        ...
        cout << ">>> ";
        getline(cin, bufWrite);
        int send = sendto(out, bufWrite.c_str(), bufWrite.size() + 1, 0, (sockaddr*)&server, sizeof(server));
